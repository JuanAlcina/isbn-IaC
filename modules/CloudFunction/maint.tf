...
resource "google_cloudfunctions2_function" "account_management_ms" {
  name        = "account_management_ms"
  location    = var.region
  ...
  build_config {
    runtime     = "python312"
    entry_point = "accounts_api" # Set the entry point in the code  
    source {
      storage_source {
        bucket = google_storage_bucket.Cloud_function_bucket.name
...
resource "google_cloudfunctions2_function_iam_member" "allow_unauthenticated" {
  cloud_function = google_cloudfunctions2_function.account_management_ms.name
